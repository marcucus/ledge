...
/// Combinaison de touches persistable pour un raccourci global (Carbon `RegisterEventHotKey`).
public struct GlobalKeyboardShortcut: Codable, Equatable, Sendable {
    ...
    // MARK: — Défauts (raccourcis historiques de Ledge)

    public static let defaultOpenClose = GlobalKeyboardShortcut(
        keyCode: UInt32(kVK_Space), modifiers: UInt32(optionKey)
    )
    public static let defaultPaste = GlobalKeyboardShortcut(
        keyCode: UInt32(kVK_ANSI_V), modifiers: UInt32(optionKey)
    )
    public static let defaultNewTimer = GlobalKeyboardShortcut(
        keyCode: UInt32(kVK_ANSI_T), modifiers: UInt32(optionKey)
    )
    public static let defaultOpenMedia = GlobalKeyboardShortcut(
        keyCode: UInt32(kVK_ANSI_M), modifiers: UInt32(optionKey)
    )

    /// Vrai si la combinaison comporte au moins une touche de modification —
    /// requis pour un raccourci global, sous peine de capturer la frappe normale.
    public var hasModifier: Bool {
        modifiers & UInt32(cmdKey | optionKey | controlKey | shiftKey) != 0
    }

    /// Représentation visuelle façon macOS, ex. "⌥ Space".
